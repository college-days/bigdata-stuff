val a = sc.textFile(PathNamespace.prefix + "subset/offlinealltrainfeatures")

val featureinputpath = PathNamespace.prefix + "subset/offlinealltrainfeatures"
...
b.coalesce(1).saveAsTextFile(PathNamespace.prefix + "offlinelrfeature")

//test
val useritemFeaturePath = PathNamespace.prefix + "subset/offlinealltestfeatures"
val c = sc.textFile(useritemFeaturePath)

val d = sc.textFile(useritemFeaturePath).map {
            record =>
                val items = record.split(",")
                val useritemid = items(0)
                val features = items(1).replace("-", " ")
...
d.coalesce(1).saveAsTextFile(PathNamespace.prefix + "offlinelrtest")
