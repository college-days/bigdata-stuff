package org.give.altc.features.useritem
...
object UserItemFeatures {
    ...
    /**
     * 108一天内是否购物车里：加入购物车之后还没有进行购买为1，若有购买行为则为0；28
     * 110三天内是否购物车里：加入购物车之后还没有进行购买为1，若有购买行为则为0；30
     * @param origindata
     * @return
     */
    /*def yayayaGetUserItemFeatures(origindata: org.apache.spark.rdd.RDD[String], targetDate: String): org.apache.spark.rdd.RDD[(String, String)] = {
        val targetDateNum = targetDate.split("-").mkString("").toInt
