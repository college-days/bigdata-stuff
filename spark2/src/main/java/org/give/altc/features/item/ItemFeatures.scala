package org.give.altc.features.item

import org.apache.spark.{SparkConf, SparkContext}
import org.apache.spark.SparkContext._
import org.give.altc.CommonOP._
import org.give.altc.{PathNamespace, TimeUtils}
...
object ItemFeatures {
    ...
    def getItemFeatures(origindata: org.apache.spark.rdd.RDD[String]): org.apache.spark.rdd.RDD[String] = {
        //一个商品 点击总次数 收藏总次数 购物车总次数 购买总次数
        //2841247
        val itemBehaviors = origindata.map {
            record =>
        ...
        //这里出来的结果会比上面出来的结果少的原因是因为在记录中好多商品对应的根本没有浏览记录 而是直接就是购买记录或者是收藏记录
        //2835284
        val itemuserbehaviors = {
            itemvisitusercount
                .leftOuterJoin(itemfavusercount).map(operatorAfterJoin)
                .leftOuterJoin(itemcartusercount).map(operatorAfterJoin)
                .leftOuterJoin(itembuyusercount).map(operatorAfterJoin)
        }
        ...
        //82343
        val buyItemUserCountWithPredicate = {
            buyItemOneTime
                .leftOuterJoin(buyItemTwoTime).map(operatorAfterJoin)
                .leftOuterJoin(buyItemThreeTime).map(operatorAfterJoin)
                .leftOuterJoin(buyItemMoreThanThreeTime).map(operatorAfterJoin)
        }
        ...
        val allItemFeatures = {
            itemBehaviors
                .leftOuterJoin(itemuserbehaviors).map(operatorAfterJoin(_, "0-0-0-0"))
                .leftOuterJoin(itemUserBuyCountAvg).map(operatorAfterJoin(_, "0"))
                .leftOuterJoin(buyItemUserCountWithPredicate).map(operatorAfterJoin(_, "0-0-0-0"))
                .leftOuterJoin(itemBuyDurationAvg).map(operatorAfterJoin(_, "0"))
        }.map {
            record =>
                record._1 + "," + record._2
        }
    ...
    def main(args: Array[String]): Unit = {
        ...
        def getFeatureData(origindata: org.apache.spark.rdd.RDD[String], output: String): Unit = {
        ...
        getFeatureData(offlinetrainorigindata, PathNamespace.offlineitemtrainfeaturesubdata)
        getFeatureData(offlinetestorigindata, PathNamespace.offlineitemtestfeaturesubdata)
        getFeatureData(onlinetrainorigindata, PathNamespace.onlineitemtrainfeaturesubdata)*/

        yayayaGetItemFeatures(offlinetrainorigindata, "2014-12-17")
    }
}