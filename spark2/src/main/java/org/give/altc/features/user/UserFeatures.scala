package org.give.altc.features.user
...
object UserFeatures {
    ...
    def main(args: Array[String]) {
        ...
        /*getUserFeatures(offlinetrainorigindata).join(yaGetUserFeatures(offlinetrainorigindata, "2014-12-17")).map(operatorAfterInnerJoin).coalesce(1).saveAsTextFile(PathNamespace.offlineusertrainfeaturesubdata)
        getUserFeatures(offlinetestorigindata).join(yaGetUserFeatures(offlinetestorigindata, "2014-12-18")).map(operatorAfterInnerJoin).coalesce(1).saveAsTextFile(PathNamespace.offlineusertestfeaturesubdata)
        getUserFeatures(onlinetrainorigindata).join(yaGetUserFeatures(onlinetrainorigindata, "2014-12-19")).map(operatorAfterInnerJoin).coalesce(1).saveAsTextFile(PathNamespace.onlineusertrainfeaturesubdata)*/
        ...
        //yaGetUserFeatures(offlinetrainorigindata, "2014-12-17")
    }
}
